...
function listTagsHelperFactory(tags, options) {
  tags = filterTags(tags, this.page.lang);
  const transformArgs = () => {
    if (!options && (!tags || !Object.prototype.hasOwnProperty.call(tags, 'length'))) {
      options = tags;
      tags = this.site.tags;
...
function filterTags(tags, lang) {
  ...
  let res = {};
  res.data = filteredTags;
  res.length = filteredTags.length;
  res.toArray = function () {
    return this.data;
  };
  res.sort = function (orderby, order) {
...
hexo.extend.helper.register('list_my_tags', listTagsHelperFactory);
